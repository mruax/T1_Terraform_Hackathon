...
class SensitiveDataPlugin(LogPlugin):
    ...
    def _sanitize_dict(self, data: Any) -> Any:
        """Recursively sanitize dictionary or list"""
        # Handle lists
        ...
        if not isinstance(data, dict):
            if isinstance(data, str):
                if self.TOKEN_PATTERN.search(data) or self.KEY_PATTERN.search(data):
...
class HTTPBodyCompressionPlugin(LogPlugin):
    ...
    def _compress_body(self, body: Any) -> Any:
        ...
        if isinstance(body, dict):
            ...
            compressed["_summary"] = f"[Array with {len(body)} items]"
...
class TerraformLogParser:
    ...
    def extract_http_bodies(self, json_obj: Dict[str, Any]) -> Tuple[Optional[Any], Optional[Any]]:
        http_req = None
        http_resp = None
...
{"@level":"error","@message":"Authentication failed","api_token":"secret123","@timestamp":"2025-09-09T15:31:33.000000+03:00"}'''
    ...
    print("uvicorn terraform_log_parser:app --reload")
